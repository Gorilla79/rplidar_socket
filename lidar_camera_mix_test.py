...
# UDP Configuration
CAMERA_HOST = '0.0.0.0'  # Listen on all interfaces for camera
CAMERA_PORT = 5000       # Camera port

LIDAR_HOST = '0.0.0.0'   # Listen on all interfaces for LiDAR
LIDAR_PORT = 5001        # LiDAR port

# Visualization setup
fig, (ax_lidar, ax_camera) = plt.subplots(1, 2)
sc = ax_lidar.scatter([], [], s=1)  # Scatter plot for LiDAR points
ax_lidar.set_xlim(-1000, 1000)
ax_lidar.set_ylim(-1000, 1000)
...
def lidar_server():
    ...
    while True:
        try:
            data, _ = server_socket.recvfrom(65536)  # Receive data
            with lidar_lock:
...
def camera_server():
    ...
    while True:
        try:
            ...
            with camera_lock:
                camera_frame = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)
        except Exception as e:
